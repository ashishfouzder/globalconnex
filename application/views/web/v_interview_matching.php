...
<section class="page-title" style="background-image: url(<?php echo base_url(); ?>others/images/background/page-title.jpg);">
    <div class="auto-container">
        <div class="title-outer">
            <h1 class="title">Interview&nbsp;Matching</h1>
        </div>
    </div>
</section>
<?php
foreach($InterviewMatching as $InterviewMatchingData)
{
    $image=$InterviewMatchingData['image'];
    $title=$InterviewMatchingData['title'];
    $text=$InterviewMatchingData['text'];

    $string2 = substr($text, 0, 729);
    $string3 = substr($text, 729, 1500);
}
